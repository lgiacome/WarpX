...
WarpX::ComputeFaceExtensions(){
    ...
    unsigned int N_ext_faces_after = CountExtFaces();
    std::cout<< "Cells to be extended after:\t" << N_ext_faces_after <<std::endl;
}
...
WarpX::ComputeOneWayExtensions() {
    ...
    for (amrex::MFIter mfi(*Bfield_fp[maxLevel()][2]); mfi.isValid(); ++mfi) {
        ...
        int temp_ind_borrowing = 0;
        int temp_ind_lending = 0;

        amrex::LoopOnCpu(box,
                         [=, &borrowing_x, &lending_x,
                             &temp_ind_borrowing, &temp_ind_lending](int i, int j, int k) {
                               ...
                               for (int k_n = -1; k_n < 2; k_n++) {
                                   ...
                                   if( !(j_n == k_n or j_n == -k_n)){
                                       ...
                                       if (Sx_red(i, j + j_n, k + k_n) > Sx_ext
                                           ...
                                           borrowing_inds_x(i, j, k).push_back(temp_ind_borrowing);
                                           temp_ind_borrowing = temp_ind_borrowing + 1;
                ...
                for (int j_n = -1; j_n < 2; j_n++) {
                    if(!(i_n == j_n or i_n == -j_n)){
                        if (Sz_red(i + i_n, j + j_n, k) > Sz_ext
                            ...
                            borrowing_inds_z(i, j, k).push_back(temp_ind_borrowing);
                            temp_ind_borrowing = temp_ind_borrowing + 1;
                            ...
                            lending_inds_z(i + i_n, j + j_n, k).push_back(temp_ind_lending);
                            temp_ind_lending = temp_ind_lending + 1;
            ...
            for (int i_n = -1; i_n < 2; i_n++) {
                for (int k_n = -1; k_n < 2; k_n++) {
                    ...
                    if (Sy_red(i + i_n, j, k + k_n) > Sy_ext
                        ...
                        borrowing_inds_y(i, j, k).push_back(temp_ind_borrowing);
                        temp_ind_borrowing = temp_ind_borrowing + 1;
                        ...
                        lending_inds_y(i + i_n, j, k + k_n).push_back(temp_ind_lending);
                        temp_ind_lending = temp_ind_lending + 1;
